...
public class FrontBookkeeper61853 implements IFrontBookkeeper {
		...
		for (int i = 0; i < news.length; i++) {
			...
			if (newsElements.length == 2 && newsElements[0].equals("show")) {
				Result += units.get(newsElements[1]).getSoldiers().toString()
						+ "\n";
			} else if (newsElements.length == 3
					&& newsElements[0].equals("show")) {
				unitDisplay(Integer.parseInt(newsElements[2]));
			} else if (newsElements.length == 4
					&& newsElements[1].equals("attached")
					&& newsElements[2].equals("to")) {
				unitAttachment(newsElements[0], newsElements[3]);
			} else if (newsElements.length == 7
					&& newsElements[1].equals("attached")
					&& newsElements[2].equals("to")) {
				unitPositionalAttachment(Integer.parseInt(newsElements[6]),
						newsElements[0], newsElements[3]);
			} else if (newsElements.length == 6
					&& newsElements[0].equals("soldiers")) {
	...
	private void unitAssignment(String[] unitArray) {
		...
		if (!Character.toString(unitArray[1].charAt(1)).equals("]")) {
			String[] soldiers = unitArray[1].substring(1,
					unitArray[1].length() - 1).split(",");
	...
	public void unitAttachment(String attachedUnit, String unit) {
		...
		if (units.get(attachedUnit).getUnitsConnected() == 1) {
			units.get(attachedUnit)
					.getCurrentUnitConnected()
					.getSoldiers()
					.removeAll(
							units.get(attachedUnit).getCurrentUnitConnected()
									.getSoldiers());
			units.get(attachedUnit).setUnitsConnected(0);
		}
		for (int i = 0; i < units.get(attachedUnit).getSoldiers().size(); i++) {
			units.get(unit).getSoldiers()
					.addLast(units.get(attachedUnit).getSoldiers().get(i));
		}
		units.get(attachedUnit).setUnitsConnected(1);
		units.get(attachedUnit).setCurrentUnit(units.get(unit));
	}

	public void unitPositionalAttachment(int id, String attachedUnit,
			String unit) {

		int size = units.get(unit).getSoldiers().size()
				- units.get(unit).getSoldiers().indexOf(id) - 1;
		...
		for (int i = 0; i < units.get(attachedUnit).getSoldiers().size(); i++) {
			units.get(unit).getSoldiers()
					.add(start, units.get(attachedUnit).getSoldiers().get(i));
			start++;
		}
		units.get(attachedUnit).setUnitsConnected(1);
		units.get(attachedUnit).setCurrentUnit(units.get(unit));
	...
	public void unitDisplay(int u) {
		...
		String result = unitDisplay.toString().replace('[', ' ')
				.replace(']', ' ');
		Result += result.trim() + "\n";
	}
