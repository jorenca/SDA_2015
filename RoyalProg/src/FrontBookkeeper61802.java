            ...
            if (words[1].equals("=")) {
                ...
                if (words.length > 3) {
                    for (int i = 2; i < words.length; i++) {
                        ...
                        String soldierId;
                        if (i == 2) {
                            soldierId = words[i].substring(1, words[i].length() - 1);
                            curSoldier = new Soldier61802(soldierId, unitName);
                        }
                        else {
                            soldierId = words[i].substring(0, words[i].length() - 1);
                            curSoldier = new Soldier61802(soldierId, unitName);
                        }
                    ...
                    if (lenght > 2) {
                        ...
                        Soldier61802 curSoldier = new Soldier61802(soldierId, unitName);
                        soldiersInUnit.add(curSoldier);
                        soldiers.put(soldierId, curSoldier);
                        units.put(unitName, new Unit61802(unitName, soldiersInUnit));
                    }
                ...
                if (words.length == 2) {
                    ...
                    LinkedList<Soldier61802> soldiersInUnit = units.get(unitName).GetSoldiers();
                    ...
                    if (soldierToShow != null) {
                        LinkedList<String> unitsAttachedTo = soldierToShow.getUnitsPartOf();
                ...
                if (words.length == 4) {
                    ...
                    upperUnit.attachUnitAfterSoldier(lowerUnit, soldiers.get(words[6]));
                }
            }
                ...
                LinkedList<Soldier61802> deadSoldiers = units.get(words[3]).removeDiedSoldiers(startSoldier, endSoldier);
...
class Unit61802 {
    ...
    private void attachmentPlaceFinderIteration(LinkedList<Soldier61802> soldiersToAdd, boolean attachAtTheEnd) {
        ...
        do {
            ...
            if (hasUpper) {
                if (hasSoldiers) {
                    curUnit.upperUnit.addSoldiersAfterSoldierIteration(soldiersToAdd, afterSoldier);
                    break;
                }
                else if (curUnit.upperUnit.soldiers.isEmpty()
                        || curUnit.equals(curUnit.upperUnit.lowerUnits.getLast())) {
                    attachAtTheEnd = true;
                }
                else if (curUnit.equals(curUnit.upperUnit.lowerUnits.getFirst())) {
    ...
    private void addSoldiersAfterSoldierIteration(LinkedList<Soldier61802> soldiers, Soldier61802 afterSoldier) {
        Unit61802 curUnit = this;
        do {
    ...
    public LinkedList<Soldier61802> removeDiedSoldiers(String startSoldier, String endSoldier) {
    ...
    private void removeDiedSoldiersInLowerUnits(LinkedList<Soldier61802> diedSoldiers) {
